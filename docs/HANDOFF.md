...
- [ ] **Google Play Developer account** — $25 one-time registration if not already done, plus Google's identity verification (has gotten stricter over time; may need ID/org documents — check current requirements when you get there).
- [ ] **Fill the two placeholders in `docs/legal/privacy_policy.md`**: a contact email, and where it'll be hosted (a public URL — a GitHub Pages page off this repo, a simple static site, whatever's easiest; Play just needs *some* stable public URL).
- [ ] **Release signing keystore** — none exists yet (`android/app/build.gradle` currently signs release builds with the *debug* key, which won't work for a real Play submission). This is sensitive and **unrecoverable if lost** — losing it means losing the ability to ever update the app again under the same listing. Needs a joint session: decide key alias, validity period, org details for the cert, and — critically — where the resulting `.jks` file and its passwords get backed up *outside* this repo (never commit it; `key.properties` should be gitignored). Claude can run `keytool` locally once these decisions are made, but the backup/custody decision is the user's alone.
